...
class Roombooking:
    def __init__(self, root):
        ...
        #===============================fetch data button================================
        btnFetchData = Button(labelframeleft, command=self.Fetch_contact,text="Fetch Data",font=("arial", 9, "bold"), bg="black", fg="sky blue", width=8)
        ...
        combo_Roomtype = ttk.Combobox(labelframeleft,font=("arial", 13, "bold"),textvariable=self.var_roomtype, width=27, state="readonly")
        ...
        combo_RoomNo = ttk.Combobox(labelframeleft,font=("arial", 13, "bold"),textvariable=self.var_roomavailable, width=27, state="readonly")
        ...
        txtTotalcost = ttk.Entry(labelframeleft,textvariable=self.var_totalcost,width=29, font=("arial", 13, "bold"))
        txtTotalcost.grid(row=9, column=1)

        #==============================bill button===================================
        btnBill = Button(labelframeleft, text="Bill",command=self.total,font=("arial", 11, "bold"), bg="black", fg="sky blue", width=10)
        ...
        #================================rightside image==========================================
        img3 = Image.open(r"C:\Users\chandana chandu\Downloads\bedroom2.webp")
        img3 = img3.resize((540, 300), Image.LANCZOS)
        self.photoimg3 = ImageTk.PhotoImage(img3)

        lblimg_logo = Label(self.root, image=self.photoimg3, bd=4, relief=RIDGE)
        lblimg_logo.place(x=750, y=55, width=540, height=300)
        ...
        self.room_table = ttk.Treeview(details_table, columns=("Contact", "Check_in", "Check_out", "Room_type", "Room_available", "Meal",
                                                                        "No_of_days"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.room_table.heading("Check_in", text="Check_out")
        self.room_table.heading("Check_out", text="Check_out")
        self.room_table.heading("Room_type", text="Room_type")
        self.room_table.heading("Room_available", text="Room_available")
    ...
    def update(self):
        if self.var_contact.get() == "":
            ...
            conn = mysql.connector.connect(host="127.0.0.1", username="root", password="", database="management")
            my_cursor = conn.cursor()
            my_cursor.execute("""Update room set Check_in=%s, Check_out=%s, Room_type=%s, Room_available=%s, Meal=%s,
                No_of_days=%s where Contact=%s
            """, (
                self.var_checkin.get(),
                self.var_checkout.get(),
                self.var_roomtype.get(),
    ...
    def Fetch_contact(self):
        if self.var_contact.get()=="":
            ...
            if row==None:
                messagebox.showerror("Error", "This Contact Not Found", parent=self.root)
            else:
                conn.commit()
                conn.close()
